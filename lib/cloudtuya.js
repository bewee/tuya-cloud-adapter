...
class CloudTuya {
  constructor(options) {
    ...
    config.region = (config.region && (config.region.toLowerCase() in ['az', 'eu', 'ay'])) ? config.region : 'eu';
  ...
  static lmap(itemState) {
    if ((typeof itemState === 'number') && (itemState === 0 || itemState === 1)) {
      return itemState;
    }
    ...
    if (typeof itemState === 'string') {
      return (['on', 'true', '1'].includes(itemState.toLowerCase()) && 1) || 0;
    }
    return itemState;
  }*/
  ...
  async state(options) {
    ...
    const states = {};
    //const returnMap =
    await devices.map((device) => (this.updateStatesCache(device.id, device.data.state, states)));
